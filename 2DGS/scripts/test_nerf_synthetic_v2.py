#!/usr/bin/env python3
# Testing script for the NeRF Synthetic dataset
...
parser = argparse.ArgumentParser(description="Parallel testing script for NeRF Synthetic (multiple checkpoints)")
...
# Create jobs list: scene names
jobs = list(scenes)


def test_scene(gpu, scene, log_file):
    """Evaluate a single NeRF Synthetic scene at multiple checkpoint iterations in-process (no subprocess calls)."""
    ...
    if args.dry_run:
        log(f"[GPU {gpu}] (dry-run) Would evaluate {scene} at iterations {iterations}")
        return True
